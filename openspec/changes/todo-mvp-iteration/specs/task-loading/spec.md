...
The client SHALL load the 100 most recently completed tasks on startup (ordered by completion date descending). Additional done tasks are loaded on demand via infinite scroll in the history view.

#### Scenario: Done tasks load with initial cap of 100

- **WHEN** the app initializes
- **THEN** the client fetches the 100 most recently completed tasks ordered by completion date descending
